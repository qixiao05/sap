...
/**
 * @program: com.edu.starter
 * @description: 时间类 工具  by1.8
 * @author: Mike
...
public class DateUtil8 {
    ...
    public static String getNowDate_yyyymmdd() {
        return LocalDate.now().format(yyyyMMdd_EN);
    }
    ...
    public static String getAfterOrPreNowTime(String date,String node, Long num) {
        LocalDateTime now = LocalDateTime.parse(date,yyyyMMddHHmmss_EN);
        if (HOUR.equals(node)) {
            return now.plusHours(num).format(yyyyMMddHHmmss_EN);
        } else if (DAY.equals(node)) {
            return now.plusDays(num).format(yyyyMMddHHmmss_EN);
    ...
    public static String getAfterOrPreNowTimePlus(String date,DateTimeFormatter dtf, String node, Long num) {
     ...
     * @param node  时间节点元素（日周月年）
     * @param index 之前之后第index个日期
     * @return yyyy-MM-dd HH:mm:ss 日期字符串
     * @author zero 2019/03/31
     */
    public static String getAfterOrPreDate_YYYYMMDDHHMMSS(String date, String node, int index) {
        if (DAY.equals(node)) {
            return LocalDateTime.parse(date,yyyyMMddHHmmss_EN).plus(index, ChronoUnit.DAYS).format(yyyyMMddHHmmss_EN);
        } else if (WEEK.equals(node)) {
            return LocalDateTime.parse(date,yyyyMMddHHmmss_EN).plus(index, ChronoUnit.WEEKS).format(yyyyMMddHHmmss_EN);
        } else if (MONTH.equals(node)) {
            return LocalDateTime.parse(date,yyyyMMddHHmmss_EN).plus(index, ChronoUnit.MONTHS).format(yyyyMMddHHmmss_EN);
        } else if (YEAR.equals(node)) {
            return LocalDateTime.parse(date,yyyyMMddHHmmss_EN).plus(index, ChronoUnit.YEARS).format(yyyyMMddHHmmss_EN);
        } else {
            return "Wrong date format!";
        }
    }
     ...
     * @return
     */
    public static boolean judgeCurrentDate(String type, Date date){
        //判断周一
        if(type.equals("week")){
    ...
    public static String getDayOfTheWeek() {
        ...
        String weekDay = String.valueOf(currentDate.getDayOfWeek());
        return  weekDay;
    }
     ...
     * 时间格式为： 自行传入
     */
    public static String timeToString(Long time, DateTimeFormatter formatString){
    ...
    public static Long getTimeStamp_YYYYMMDD(String date){
    ...
    public static Long getTimeStamp_YYYYMMDD1(String date){
     ...
     * 获取指定日期天数
     */
    public static int getCurrentMonthTotalDay(String dateStr) throws Exception {
     ...
     * 获取指定日期月份
     */
    public static int getCurrentMonth(String dateStr) throws Exception {
        Date date = new SimpleDateFormat("yyyy-MM-dd").parse(dateStr );
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
     ...
     * 获取指定日期号数
     */
    public static int getCurrentDay(String dateStr) throws Exception {
        Date date = new SimpleDateFormat("yyyy-MM-dd").parse(dateStr );
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
     ...
     * 获取指定日期年份
     */
    public static int getCurrentYear(String dateStr) throws Exception {
        Date date = new SimpleDateFormat("yyyy-MM-dd").parse(dateStr );
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
